...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        protected override void DrawPropertyLayout(IPropertyValueEntry<TArray> entry, GUIContent label)
        {
            ...
            if (context.Value == null || colCount != context.Value.ColCount || rowCount != context.Value.RowCount)
            {
                ...
                context.Value.Attribute = entry.Property.Info.GetAttribute<TableMatrixAttribute>() ?? new TableMatrixAttribute();
        ...
        where TElement : UnityEngine.Object
    {
        protected override TElement DrawElement(Rect rect, TElement value)
        {
            ...
            value = DragAndDropUtilities.DragZone(rect, value, true, true, id);
            value = DragAndDropUtilities.ObjectPickerZone(rect, value, false);

            return value;
        }

        private static TValue DrawSquareObjectField<TValue>(Rect rect, TValue value, GUIContent label)
        {
            var id = DragAndDropUtilities.GetDragAndDropId(rect);

            DragAndDropUtilities.DrawDropZone(rect, value, label, id: id);
            value = DragAndDropUtilities.DropZone(rect, value, id);
            value = DragAndDropUtilities.DragZone(rect, value, true, true, id);
            value = DragAndDropUtilities.ObjectPickerZone(rect, value, false);
