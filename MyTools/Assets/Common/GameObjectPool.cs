...
public class GameObjectPool : SingletonBehaviour<GameObjectPool>
{
    ...
    public void PoolClear(string key)
    {
        if (cache.ContainsKey(key))
        {
            cache[key].Clear();
    ...
    public GameObject PoolCreateObject(string key, GameObject go, Vector3 position, Quaternion quaternion)
    {
        //缓存中有空闲的物体，返回空闲
    ...
    public void PoolDestory(GameObject destoryGo)
    {
        destoryGo.SetActive(false);
    }

    /// <summary>延迟销毁</summary>
    public void PoolDestory(GameObject destoryGo, float delay)
    {
        //开启协程，控制延迟的时间
