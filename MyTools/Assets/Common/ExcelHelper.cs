...
namespace Microsoft.Office.Interop.ExcelEdit
{
    ...
    public class ExcelHelper
    {
        ...
        /// <returns></returns>
        public string Read(string ws, int x, int y)
        {
            wbs = app.Workbooks;
            Microsoft.Office.Interop.Excel.Range range = GetSheet(ws).Range[GetSheet(ws).Cells[x, y], GetSheet(ws).Cells[x, 42]];
        ...
        public void Open(string FileName)//打开一个Microsoft.Office.Interop.Excel文件
        {
            ...
            wbs = app.Workbooks;
            //wb = wbs.Add(FileName);
            //wb = wbs.Open(FileName, 0, false, 5, "", "", true, Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "t", false, false, 0, true, Type.Missing, Type.Missing);
