...
namespace PixelCrushers.DialogueSystem.Aurora
{
    ...
    public class AuroraConverterWindow : EditorWindow
    {
        ...
        [MenuItem("Window/Dialogue System/Converters/Aurora (Neverwinter Nights) Converter", false, 1)]
        ...
        private void DrawProfilesSection()
        {
            EditorWindowTools.DrawHorizontalLine();
            EditorGUILayout.BeginHorizontal();
            profilesFoldout = EditorGUILayout.Foldout(profilesFoldout, "Converter Profiles", BoldFoldoutStyle);
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("?", EditorStyles.miniButton, GUILayout.Width(22)))
            {
            ...
            {
                EditorWindowTools.StartIndentedSection();
                if (profilesHelp) EditorGUILayout.HelpBox("You can save and load converter settings as XML files here. This allows you to switch between different sets of Aurora files without losing your settings.", MessageType.Info);
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
        ...
        private void DrawConvertButton()
        {
            ...
            if (GUILayout.Button("Convert", GUILayout.Width(100))) ConvertAuroraFiles();
            EditorGUI.EndDisabledGroup();
        }
        ...
        {
            string filename = EditorUtility.SaveFilePanel("Save Profile XML File", "", "Aurora Converter Profile", "xml");
            if (!string.IsNullOrEmpty(filename))
            {
        ...
        public void ConvertAuroraFiles()
        {
            try
            {
                ...
                {
                    EditorUtility.DisplayProgressBar("Converting Aurora Files", "Please wait...", 0);
        ...
        {
            EditorUtility.DisplayProgressBar("Converting Dialog Files", string.Format("Configuring localization in {0}...", conversation.Title), 1);
