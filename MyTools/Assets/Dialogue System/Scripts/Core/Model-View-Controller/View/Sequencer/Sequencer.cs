...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class Sequencer : MonoBehaviour
    {
        ...
        {
            get { return originalOrthographicSize; }
        }
        ...
        /// <summary>
        /// Set <c>true</c> to use the old Dialogue System parser instead of the new, 
        /// improved parser.
        /// </summary>
        public bool useOldParser = false;
        ...
        private void InformParticipants(string message)
        {
            ...
            if (DialogueManager.Instance.transform != speaker && DialogueManager.Instance.transform != listener)
            {
        ...
        public void PlaySequence(string sequence)
        {
            ...
            if (!useOldParser) // Use new, more robust parser:
            {
            ...
            else // Use old parser:
            {
                ...
                foreach (string statement in statements)
                {
                    ...
                    string[] tokens = System.Text.RegularExpressions.Regex.Split(statement, @"\s*\(|\)|\s*\@");
                    ...
                    string message = null;
                    if (tokens.Length >= 4)
                    { // tokens[2] is an empty token between ")@".
        ...
        public void PlayCommand(QueuedSequencerCommand commandRecord, string commandName, bool required, float time, string message, string endMessage, params string[] args)
        {
            ...
            else
            {
                if (commandRecord != null)
                {
                    ...
                    queuedCommands.Add(commandRecord);
                }
                else
                {
        ...
        private bool HandleSendMessageInternally(string commandName, string[] args)
        {
            ...
            Transform subject = everyone ? DialogueManager.Instance.transform
                : SequencerTools.GetSubject(SequencerTools.GetParameter(args, 2), speaker, listener);
        ...
        private bool HandleSetPortraitInternally(string commandName, string[] args)
        {
            ...
            else if (isPicTag)
            {
                ...
                texture = actor.GetPortraitTexture(picNumber);
            }
            else
            {
        ...
        {
            Debug.Log(string.Format("{0}: Sequencer: UpdateTracker()", new System.Object[] { DialogueDebug.Prefix }));
