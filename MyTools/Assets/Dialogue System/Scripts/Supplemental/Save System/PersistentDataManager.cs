    ...
    public static class PersistentDataManager
    {
        ...
        public static void AppendConversationData(StringBuilder sb)
        {
            ...
            try
            {
				...
				//foreach (KeyValuePair<object, object> conversationPair in conversationTable.luaTable) {
				//	var conversationID = conversationPair.Key;
				//	sb.AppendFormat("Conversation[{0}].Dialog = {{ ", conversationID);
				//	//Debug.Log ("<color=cyan>Conv: " + conversationID + "</color>");
				//	var dialogTable = (conversationPair.Value as NLua.LuaTable)["Dialog"] as NLua.LuaTable;
				//	foreach (KeyValuePair<object, object> dialogPair in dialogTable) {
				//		var dialogID = dialogPair.Key;
				//		var simStatus = (dialogPair.Value as NLua.LuaTable)["SimStatus"];
				//		//Debug.Log ("   " + dialogID + ": " + simStatus);
				//		sb.AppendFormat("[{0}]={{SimStatus=\"{1}\"}},", new System.Object[] { dialogID, simStatus });
				//	}
				//	sb.Append("}; ");
				//}
#else
