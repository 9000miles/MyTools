...
namespace PixelCrushers.DialogueSystem
{
    ...
    public abstract class AbstractDialogueUI : MonoBehaviour, IDialogueUI
    {
        ...
        public virtual void ShowAlert(string message, float duration)
        {
            if (!IsOpen)
            {
                if (!hasOpenedBefore) Dialogue.Hide();
                UIRoot.Show();
            }
        ...
        {
            if (IsOpen) DialogueManager.Instance.SendMessage("OnConversationContinue", (IDialogueUI)this, SendMessageOptions.DontRequireReceiver);
        }
